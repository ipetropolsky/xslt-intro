...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="yml_catalog">
		...
		<html>
			<head>
				...
				/>
				<title><xsl:text>Каталог</xsl:text></title>
			</head>
			<body>
				<div class="container-fluid">
					<h1><xsl:text>Каталог</xsl:text></h1>
	...
	<xsl:template match="offer" mode="offerBody">
		<div class="card-body">
			...
			<div class="text-muted font-weight-light"><xsl:value-of select="vendorCode" /></div>
			<h5 class="card-title">
				<a href="{url}" title="Перейти в магазин" target="_blank">
					<xsl:value-of select="name" />
				</a>
			</h5>
			<p class="card-text" style="max-height: 6em; overflow:hidden;"><xsl:value-of select="description" /></p>
